...
from ranker.utils.turbovec import save_turbovec_embeddings
...
def build_embeddings(candidates, cache_path: str, ids_path: str, model_name: str, batch_size: int):
    ...
    save_turbovec_embeddings(embeddings, cache_path)
    save_json(candidate_ids, ids_path)
    log.info("Embeddings compressed in %.1fs → shape %s", time.time() - t0, embeddings.shape)
...
def build_jd_embedding(model_name: str, cache_path: str) -> np.ndarray:
    ...
    model = SentenceTransformer(model_name)
    jd_vec = model.encode([JD_TEXT_KEY_SECTIONS], normalize_embeddings=False)[0].astype(np.float32)
    jd_vec = jd_vec / (np.linalg.norm(jd_vec) + 1e-8)
...
def main():
    ...
    embed_path = str(cache / "candidate_embeddings.turbovec.npz")
